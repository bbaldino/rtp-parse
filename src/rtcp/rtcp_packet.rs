...
pub fn parse_single_rtcp_packet<B: PacketBuffer>(buf: &mut B) -> Result<SomeRtcpPacket> {
    println!("Parsing single rtcp packet, buf: {buf:x}");
    ...
    let result = match header.packet_type {
        ...
        RtcpFbPsPacket::PT | RtcpFbTlPacket::PT => {
            ...
            match header.packet_type {
                RtcpFbPsPacket::PT => match header.report_count {
                    RtcpFbFirPacket::FMT => Ok(SomeRtcpPacket::RtcpFbFirPacket(
                        read_rtcp_fb_fir(&mut payload_buffer, header, fb_header)
                            .context("rtcp fb fir")?,
                    )),
                    fmt => bail!("Unsupposrted RtcpFbPs packet fmt {fmt}"),
                },
                RtcpFbTlPacket::PT => bail!("TODO: RtcpFbTl packet"),
                _ => unreachable!(),
            }
        }
